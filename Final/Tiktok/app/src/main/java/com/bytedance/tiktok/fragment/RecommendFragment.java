...
public class RecommendFragment extends BaseFragment {
    ...
    private void playCurVideo(int position) {
        ...
        //切换播放视频的作者主页数据
        RxBus.getDefault().post(DataCreate.datas.get(position).getUserBean());
    ...
    private void autoPlayVideo(int position, ImageView ivCover) {
        if(DataCreate.datas.get(position).getVideoPath()!=null){
            videoView.setVideoPath(DataCreate.datas.get(position).getVideoPath());
        }
        else {
